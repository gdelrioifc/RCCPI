     ...
     if(args.length==0)
      {
       ...
       System.out.println("-fileList  \t<csv file with filename, chain, minDist, maxDist, lateralChain; should include -pathPdbFolder>");
...
//       if(optionIndex ==0)  System.out.println("no fileList");
       if(optionIndex>0) 
        {
         ...
         if(optionIndex == 0)
          {
//           System.out.println("No pathPdbFolder");
           Parameters.loadCorrect = false;
           return;
          }
         else Parameters.pathPdb = options.get(optionIndex);
        }
...
//         System.out.println("-chain missing");
       ...
       else Parameters.chain = options.get(optionIndex);
...
//         System.out.println("-pathTomita missing");
       ...
       else Parameters.pathTomita = options.get(optionIndex);
            
       optionIndex = options.indexOf("-lateralChain") + 1;
       if(optionIndex == 0) Parameters.flagLateralChain = false;
       else Parameters.flagLateralChain = options.get(optionIndex).equalsIgnoreCase("yes");
            
            //System.out.println("-minDist minimum contact distance (0)");
       optionIndex = options.indexOf("-minDist") + 1;
       if(optionIndex == 0) Parameters.min = 0;
       else Parameters.min = Integer.parseInt(options.get(optionIndex));
            
            //System.out.println("-maxDist maximum contact distance (5)");
       optionIndex = options.indexOf("-maxDist") + 1;
       if(optionIndex == 0) Parameters.max = 7;
       else Parameters.max = Integer.parseInt(options.get(optionIndex));
       ...
       if(Parameters.fileList!=null)
        {
         ...
         System.out.println("The rest of parameters are taken from -fileList.");
        }
       else
        {
