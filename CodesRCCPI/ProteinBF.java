   ...
   try
    {
     ...
     ProteinBF peptide = getPeptide(pep_ini, pep_len);
...
//System.err.println("Ini:"+pep_ini+", End:"+(pep_ini+pep_len)+", No. residues:"+p_residues.size()+":"+((ResidueBF)p_residues.get(0)).toString()+"-"+((ResidueBF)p_residues.get(p_residues.size()-1)).toString());
 ...
 public ProteinBF getPeptide(int pep_ini, int pep_len)
  {
   ...
   ArrayList<ResidueBF> residues=this.getResidues();
   ResidueBF r=null;
   try
    {
     String atom="", coord="";
     if(this.residues!=null)
      {
//System.err.println("Reading protein to get peptide:"+this.toString()+", protein length="+this.getProteinSize()+", residues:"+residues.size()+", ini:"+pep_ini+", end:"+(pep_ini+pep_len));
//System.err.println("Residues:"+residues.toString());
       int i=0;
       result.peptideStart=pep_ini;
       result.peptideEnd=pep_ini+pep_len;

       for(i=pep_ini; i<(pep_ini+pep_len); i++)
        {
         r=(ResidueBF)residues.get(i);
         result.addResidue(r);
